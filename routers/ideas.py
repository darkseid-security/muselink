...
from utils.gpt5_service import get_gpt5_service_for_user
from utils.flux_service import get_flux_service_for_user
from utils.voice_service import get_voice_service_for_user
from utils.kling_service import get_kling_service_for_user
...
async def generate_script_idea(
    ...
    try:
        # Get GPT-5 service for user (reads API key from database)
        gpt5_service = get_gpt5_service_for_user(user_id)
...
async def generate_image(
    ...
    try:
        # Get Flux service for user (reads API key from database)
        flux_service = get_flux_service_for_user(user_id)
...
async def generate_voice(
    ...
    try:
        # Get Voice service for user (reads API key from database)
        voice_service = get_voice_service_for_user(user_id)
...
async def generate_video(
    ...
    try:
        # Get Kling video service for user (reads API key from database)
        kling_service = get_kling_service_for_user(user_id)
